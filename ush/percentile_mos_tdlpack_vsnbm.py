...
def main():
	...
	parser.add_argument('-datafiles',type=str,dest="tdlloc",required=True,help="REQUIRED location of input tdlpack files (NBM forecasts)")
	parser.add_argument('-obfiles',type=str,dest="obloc",help="REQUIRED location of input mos forecasts!")
	...
	parser.add_argument('-compareto',type=str,dest="compare_to",choices=["obs","deterministic"],help="Compare MOS percentiles against obs or deterministic (default obs)")
	...
	if element == "maxt":
		...
		print("MOS Core valid end datetime: ",nbm_core_valid_end_datetime.strftime("%m-%d-%Y %H:%M"))
	...
	print("MOS Core Forecast hour: ",str(nbm_core_forecasthour))
	...
	region_list = [region]
	obs={}
	#here, obs file will be a gridded seq file, with output at given points
        ###################################################################
        ###############################Process MOS Data####################
        ###################################################################
	mosd=pytdlpack.open(,mode='r',format='r') #format = random=access
	#load list of stations
	stations=mosd.read(id=[400001000,0,0,0])
	if element == "maxt":
		print("MOS core forecast hour=" + str(nbm_core_forecasthour)
		mosrec=mosd.read(id=[24280066,0,int(nbm_core_forecasthour),0])
	...
	if compare_to == "deterministic":
		...
		if element == "maxt":
			print("MOS core forecast hour=" + str(nbm_core_forecasthour))
			detrec=nbmd.read(id=[242480066,0,int(nbm_core_forecasthour),0])
		...
		for region in region_list:
			print("     >> Extracting MOS deterministic and coordinate values")
			...
			obs[region]["MOS_fidx"] = nbm_fidx
			obs[region]["MOS_D"] = detr_values
	else:
	...
	if element == "maxt" or element == "mint":
		...
		#prec_file=tdlloc + "/" + hh + "z.nbm"
		perc_file=tdlloc + "/gmos." + nbm_init_date + "/" + "gmos_" + element + "_" + nbm_init_date + ".grd_ra"
	elif element == "qpf":
	...
	stations=nbmperc.read(id=[400001000,0,0,0]) #id to read stations from ra file
	#stnarr=np.array(stations)
	print('   > Extracting NBM Probabilistic') 
	for perc in perc_list:
		perci=str(perc).zfill(2)
		print("     >> Extracting MOS P" + perci)
		perc_name = "MOS_P"+str(perci)
		...
		if element == "maxt":
			percrec=nbmperc.read(id=[224500066,int(perc_str),int(nbm_qmd_forecasthour),400])
			percrec.unpack(data=True,missing_value=-9999.0)
			percdata=K_to_F(percrec.data)
		elif element == "mint":
			percrec=nbmperc.read(id=[242480066,int(perc_str),int(nbm_qmd_forecasthour),400])
			percrec.unpack(data=True,missing_value=-9999.0)
			percdata=K_to_F(percrec.data)
		#elif element == "qpf":
		...
		for region in region_list:
			#nbm_coords = obs[region]["MOS_fidx"].values
			nbm_stations= obs[region]["stid"].values
			perc_values = []
			for i in range(0, len(nbm_stations)):
				perc_value = percdata[i] #percdata[nbm_stations[i]]
...
########################################################################################################################
	...
	for region in region_list:
		perc_start = obs[region].columns.get_loc("MOS_P01")
		perc_end = obs[region].columns.get_loc("MOS_P99")
		...
		if 'NBM_D' in obs[region].index:
			all_nbmd = obs[region][['MOS_D']].values
		else:
	...
	obs[region]["MOSd_perc"] = nbmd_percs
	if export_csv:
...
########################################################################################################################
	...
	if region == "CONUS":
		...
		fig.text(0.30, 0.885, region + " " + title_dict[element][0] + " " + compare_element + " in GMOS " + title_dict[element][1] + " Percentile Space",horizontalalignment='center',weight='bold',fontsize=25,color=text_color)
...
#
	...
	#set up 2 panel plot
		...
		fig.text(0.5, 1.05,dataframeid + " " + title_dict[element][0] + " " + compare_element + " in MOS " + title_dict[element][1] + " Percentile Space",horizontalalignment='center', verticalalignment='bottom', weight='bold',fontsize=20,color=text_color)
		...
		fig.text(0.5,1.05,"Valid: " + valid_title + " | MOS Init: " + nbm_init_title + " | Points: " + points_str,horizontalalignment='center',verticalalignment='top', fontsize=16,color=text_color)
		...
		if region in ("CR","SR"):
			handles, labels = scatter.legend_elements(num=10)
			legend1 = ax1.legend(flip(handles, 6), flip(labels, 6), ncol=6,loc=lloc, title=compare_element + " in MOS Percentile Space", fancybox=True)
		else:
			legend1 = ax1.legend(*scatter.legend_elements(num=10),loc=lloc, title=compare_element + " \n Rank", fancybox=True)
		plt.setp(legend1.get_title(), multialignment='center', color=text_color)
		for text in legend1.get_texts():
		...
		ax2.set_xlabel(compare_element + " in MOS " + title_dict[element][1] + " Percentile Bins", color=text_color, fontsize=12)
